...
当前版本 **1.7.0**：创作草稿会自动保存，返回首页、刷新或切换工作区后可继续；浏览器后退与前进可在作品标签和材料详情之间往返。界面统一了卡面、按钮与导航层级，手机用料记录采用独立卡片。存储阻止旧标签页覆盖新修改，“标记完成”一并保存当前成品记录。作品材料、结构和已有图片继续保留。

- [1.7.0 更新记录](docs/release-1.7.0.md)：界面、草稿、导航与存储可靠性。
- [产品优化方案](docs/PRODUCT_OPTIMIZATION.md)：竞品取舍、逐页设计、分期与验收。
- [代码结构与维护入口](docs/ARCHITECTURE.md)：模块边界、源文件与兼容测试。
- [项目使用说明](docs/USER_GUIDE.md)：创作空间与 Studio 如何配合，从第一次使用到制作、备份与换设备。
- [1.6.2 修复记录](docs/release-1.6.2.md)：手机触摸拖动、空间调整与整个项目的使用说明。
- [1.6.1 修复记录](docs/release-1.6.1.md)：旧缓存升级与制作末步跳转。
