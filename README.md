...
当前版本 **1.5.1**：方向页预览当前 Blueprint 和六种变化，正面/俯视以及分支比较共用比例；预览不会创建项目，采用方向时仍通过原有锁定规则创建分支。重做首页及四类创意物件示意，纠正十项花材错配照片。126 张参考照片及来源固定在仓库并校验 SHA-256，另四张 AI 图明确标注为形态示意。详见 [1.5.1 发布说明](docs/release-1.5.1.md)。
