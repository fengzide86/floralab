...
当前版本 **1.5.0**：方向页直接预览当前 Blueprint 和六种变化，正面/俯视以及分支比较共用比例；预览不会创建项目，采用方向时仍通过原有锁定规则创建分支。首页与三类创意材料图片重新制作；AI 物件图明确标记为形态示意。127 张既有参考照片及来源固定在仓库并校验 SHA-256，部署不会重新搜索替换。详见 [1.5.0 发布说明](docs/release-1.5.0.md)。
