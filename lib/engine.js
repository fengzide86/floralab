...
function choosePalette(input) {
  if(input._intent)return input._intent.colors.length?[...input._intent.colors]:['白色','绿色'];
...
function applyAvoid(flowers, catalog, input) {
  const avoid = [input.avoid,...Intent.negativeClauses(input.idea||input.prompt||'')].filter(Boolean).join(' ');
...
function localPlan(catalog,input={}) {
  const normalized={...(input.intentVersion===1?Intent.normalize(catalog,input):input), mode:input.mode==='creative'?'creative':'floral'};
  if(normalized._intent?.required?.some(r=>catalog.creative.some(c=>c.id===r.id)))normalized.mode='creative';
  ...
  if(normalized._intent){
    const intent=normalized._intent;
    creative=creative.filter(c=>!intent.excludes.includes(c.name));
    for(const r of intent.required){
      let flower=flowers.find(f=>f.id===r.id),item=flower||creative.find(c=>c.id===r.id);
      if(!item){item=catalog.flowers.some(f=>f.id===r.id)?makeFlower(catalog,r.name,r.quantity||1,'指定材料',palette):makeCreative(catalog,r.name,r.quantity||1);if(!item)throw new Error('材料未能加入方案：'+r.name);if(catalog.flowers.some(f=>f.id===r.id)){flowers.push(item);flower=item;}else creative.push(item);}
      if(r.quantity!==null){item.quantity=r.quantity;item.locked=true;}
      if(r.owned&&r.quantity!==null)item.existing_quantity=r.quantity;
      if(r.color&&flower)flower.color=r.color;
      if(flower)flower.subtotal=Math.max(0,flower.quantity-(flower.existing_quantity||0))*flower.unit_price;
      else item.price=Math.max(0,item.quantity-(item.existing_quantity||0))*item.unit_price;
    }
    if(intent.colors.length)for(const f of flowers){
      const explicit=intent.required.find(r=>r.id===f.id&&r.color);
      if(!explicit){const ref=flowerByName(catalog,f.name);const color=intent.colors.map(x=>x.replace('色','')).find(x=>ref?.colors?.includes(x));if(color)f.color=color;else if(!intent.required.some(r=>r.id===f.id))f._omitForColor=true;}
    }
    flowers=flowers.filter(f=>!f._omitForColor);
      if(!flowers.length&&!creative.length)throw new Error('这些条件下没有可用的主体材料，请调整排除项或指定一种材料。');
  }
