...
function adjustBudget(flowers, creative, fixedCost, input) {
  ...
  const hard=Boolean(input.budget) && input.budgetPriority !== 'effect';
  ...
  return {flowers:cloneF.filter(x=>x.quantity>0),creative:cloneC.filter(x=>x.quantity>0),flowerCost:flowerCost(),creativeCost:creativeCost(),total:minFeasible,budget,conflict:hard && minFeasible>budget,effectOver:!hard && Boolean(input.budget) && minFeasible>budget};
}
...
function buildRuleChecks(catalog,input,flowers,creative,mechanics,cost) {
  ...
  if(cost.conflict) add('budget','预算约束','block',`指定条件下最低粗估约 ¥${cost.total}，高于预算 ¥${cost.budget}。建议提高预算、减少指定材料或缩小尺寸。`);
...
function localPlan(catalog,input={}) {
  ...
  return {
    ...
    cost:{flowers:adjusted.flowerCost,creative:adjusted.creativeCost,vessel:vesselCost,supplies:suppliesCost,total,range:[Math.round(total*.9),Math.round(total*1.15)],budget:adjusted.budget,status:adjusted.conflict?'conflict':adjusted.effectOver?'effect-over':'within',note:catalog.meta.price_note},
