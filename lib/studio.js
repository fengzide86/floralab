...
function createVariation(catalog,rawPlan,presetKey,options={}){
  ...
  const plan=Exploration.fork(original,{label:options.label||preset.label});
  const applied=[];
  ...
  plan.exploration.lastVariation={preset:preset.key,label:preset.label,applied,from_branch:original.exploration.branch.id,at:now()};
...
function validateImportObject(raw,catalog){
  ...
  if(schema && ![V.HANDOFF_SCHEMA,V.LEGACY_HANDOFF_SCHEMA].includes(schema))warnings.push(`未知 schema：${schema}，将尝试兼容导入。`);
...
function migratePlan(catalog,raw){
  ...
  if(schema===V.LEGACY_HANDOFF_SCHEMA||plan.engineVersion===V.LEGACY_ENGINE_VERSION){
    plan.engineVersion=V.ENGINE_VERSION;plan.recipe=Array.isArray(plan.recipe)?plan.recipe:buildRecipe(plan);syncPlanFromRecipe(plan);enrichChecks(plan,catalog);plan.blueprint=buildBlueprint(catalog,plan,plan.blueprint);compositionCheck(plan);initBuild(plan);plan.resultFeedback=plan.resultFeedback||null;plan.history=Array.isArray(plan.history)?plan.history:[];Exploration.ensure(plan);plan.handoff={...(plan.handoff||{}),schema:V.HANDOFF_SCHEMA,version:Number(plan.handoff?.version||1),updated_at:now()};appendHistory(plan,'migration','从 0.5 迁移到 1.0');
  } else {
    plan.engineVersion=V.ENGINE_VERSION;plan.handoff={...(plan.handoff||{}),schema:V.HANDOFF_SCHEMA,version:Number(plan.handoff?.version||1),updated_at:plan.handoff?.updated_at||now()};if(!plan.blueprint||plan.blueprint.version!==V.BLUEPRINT_VERSION)plan.blueprint=buildBlueprint(catalog,plan,plan.blueprint);compositionCheck(plan);initBuild(plan);plan.history=Array.isArray(plan.history)?plan.history:[];Exploration.ensure(plan);
  }
  return plan;
}

function handoffObject(plan){return {schema:V.HANDOFF_SCHEMA,project:{id:plan.id,title:titleOf(plan),version:plan.handoff?.version||1},intent:{mode:plan.mode,type:plan.type,occasion:plan.occasion,style:plan.style,palette:plan.palette,budget:plan.request?.budget||plan.cost?.budget,avoid:plan.request?.avoid||'',pet_context:plan.request?.petContext||''},assessment:plan.assessment,composition:plan.composition,compositionIntent:plan.compositionIntent||null,exploration:plan.exploration,recipe:plan.recipe,dimensions:plan.dimensions,mechanics:plan.mechanics,blueprint:plan.blueprint,render:buildRenderSpec(plan),build:plan.build,resultFeedback:plan.resultFeedback,history:plan.history,plan};}
