...
function publicCatalog(catalog){return {meta:catalog.meta,flowers:catalog.flowers.map(f=>({id:f.id,name:f.name,aliases:f.aliases||[],colors:f.colors||[],roles:f.roles||[],price:f.price,season:f.season,availability:f.availability,stem_cm:f.stem_cm||null,head_cm:f.head_cm||null,weight:f.weight||'',stem_strength:f.stem_strength||'',water_need:f.water_need||'',fragility:f.fragility||'',beginner:Boolean(f.beginner),foam_fit:f.foam_fit||'',vase_fit:f.vase_fit||'',peak_months:f.peak_months||[],market_tier:f.market_tier||'',substitutes:f.substitutes||[],pet_risk:f.pet_risk||'',notes:f.notes||'',reference_only:Boolean(f.reference_only)})),creative:(catalog.creative||[]).map(c=>({id:c.id,name:c.name,aliases:c.aliases||[],category:c.category||'',price:c.price||null,weight_g:c.weight_g||null,fix:c.fix||[],wet_safe:Boolean(c.wet_safe),food:Boolean(c.food),notes:c.notes||''}))};}
