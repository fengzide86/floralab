...
const {buildRecipe,totalsFromRecipe,budgetCheck,syncPlanFromRecipe} = require('./recipe');
...
function initBuild(plan){
  ...
  for(const r of plan.recipe||[]){if(['flower','creative'].includes(r.kind)){usage[r.key]=Math.max(0,Number(usage[r.key]||0));losses[r.key]=Math.max(0,Number(losses[r.key]||0));}}
  plan.build={currentStep:Number(prev.currentStep||0),completed:Array.isArray(prev.completed)?prev.completed:[],usage,losses,notes:Array.isArray(prev.notes)?prev.notes:[],updated_at:now(),...(prev.preparation?{preparation:clone(prev.preparation)}:{}),...(prev.undo?{undo:clone(prev.undo)}:{})};
...
function updateRecipe(catalog,rawPlan,patches=[]){
  ...
  for(const p of patches){const r=byKey.get(p.key);if(!r)continue;if(p.quantity!==undefined)r.quantity=Math.max(0,Number(p.quantity)||0);if(p.owned!==undefined)r.owned=Math.max(0,Math.min(r.quantity,Number(p.owned)||0));if(p.unit_price!==undefined){r.unit_price=p.unit_price===null?null:Math.max(0,Number(p.unit_price)||0);r.price_source=p.price_source||'actual';}r.to_buy=Math.max(0,r.quantity-r.owned);r.subtotal=Number((r.to_buy*r.unit_price).toFixed(2));}
...
function replaceMaterial(catalog,rawPlan,choice){
  const plan=clone(rawPlan),row=plan.recipe.find(r=>r.key===choice.key);
  if(!row||!['flower','creative'].includes(row.kind))throw new Error('请选择一行主体材料');
  const list=row.kind==='flower'?catalog.flowers:catalog.creative,ref=list.find(r=>r.id===choice.id);
  if(!ref)throw new Error('没有找到这项材料');
  if(plan.recipe.some(r=>r.kind===row.kind&&r.id===ref.id&&r.key!==row.key))throw new Error('该材料已在清单中，请直接调整已有行');
  const color=String(choice.color||ref.colors?.[0]||ref.category||'');
  if(row.kind==='flower'&&!ref.colors.includes(color))throw new Error('这项材料尚未确认该颜色');
  const price=Array.isArray(ref.price)?Math.round((ref.price[0]+ref.price[1])/2):null;
  const next={...row,key:row.kind+':'+ref.id,id:ref.id,name:ref.name,variant:color,owned:0,unit_price:price,price_source:'reference',to_buy:row.quantity,subtotal:price===null?0:row.quantity*price};
  plan.recipe=plan.recipe.map(r=>r.key===row.key?next:r);
  const oldNodeKeys=new Set(plan.blueprint.nodes.filter(n=>n.material_id===row.id).map(n=>n.id));
  const old=plan.flowers.find(f=>f.id===row.id)||plan.creative.find(c=>c.id===row.id);
  const material={...old,id:ref.id,name:ref.name,color,category:ref.category,existing_quantity:0,unit_price:price,subtotal:next.subtotal,price:next.subtotal,catalog:clone(ref)};
  if(row.kind==='flower')plan.flowers=plan.flowers.map(f=>f.id===row.id?material:f);
  else plan.creative=plan.creative.map(c=>c.id===row.id?material:c);
  // Preserve placement while explicitly changing the material and its head size.
  const bp=clone(plan.blueprint);
  for(const n of bp.nodes)if(oldNodeKeys.has(n.id)){n.material_id=ref.id;n.name=ref.name;n.color=color;n.head_cm=ref.head_cm?Math.max(3,Math.min(15,(ref.head_cm[0]+ref.head_cm[1])/2)):n.head_cm;}
  plan.blueprint=buildBlueprint(catalog,plan,bp);revalidate(plan,catalog);
  plan.handoff={...plan.handoff,version:Number(plan.handoff?.version||1)+1,updated_at:now()};
  appendHistory(plan,'replace',`将 ${row.name} 替换为 ${color}${ref.name}，保留 ${row.quantity}${row.unit}；已有数量重新核对`);
  return plan;
}

function updateRecord(rawPlan,patch){
  const plan=Records.change(rawPlan,patch);
  plan.handoff={...plan.handoff,version:Number(plan.handoff?.version||1)+1,updated_at:now()};
  appendHistory(plan,'record',patch.title!==undefined?'修改作品名称':`阶段：${Records.STAGES[patch.stage]}`);
  if(patch.stage&&patch.stage!=='exploring')Exploration.updateLocks(plan,Object.fromEntries(Exploration.LOCK_KEYS.map(k=>[k,true])));
  return plan;
}
...
function updateBuild(catalog,rawPlan,patch={}){
  ...
  if(patch.undo){
    if(!plan.build.undo)throw new Error('没有可以撤销的制作记录');
    const previous=plan.build.undo;Object.assign(plan.build,previous);delete plan.build.undo;
  }else if(r&&(patch.used!==undefined||patch.loss!==undefined||patch.useDelta!==undefined||patch.lossDelta!==undefined)){
    plan.build.undo={usage:clone(plan.build.usage),losses:clone(plan.build.losses),currentStep:plan.build.currentStep,completed:clone(plan.build.completed)};
  }
  ...
  if(['tools','materials','water'].includes(patch.prepared))plan.build.preparation={...(plan.build.preparation||{}),[patch.prepared]:Boolean(patch.checked)};
...
function validateImportObject(raw,catalog){
  const errors=[],warnings=[];if(!raw||typeof raw!=='object'||Array.isArray(raw))return {ok:false,errors:['文件内容不是作品对象'],warnings:[]};
  let schema=raw.schema||raw.handoff?.schema||raw.plan?.handoff?.schema||null;let plan=raw.plan||raw;
  if(!plan||typeof plan!=='object'||Array.isArray(plan))return {ok:false,errors:['缺少作品数据'],warnings};
  if(schema && ![V.HANDOFF_SCHEMA,V.PREVIOUS_HANDOFF_SCHEMA,V.LEGACY_HANDOFF_SCHEMA].includes(schema))errors.push(`不支持的文件版本 ${schema}；请使用生成此文件的新版 Studio，原作品不会被替换。`);
  if(!plan.id||typeof plan.title!=='string'||!plan.title.trim())errors.push('缺少作品 ID 或名称');
  if(!Array.isArray(plan.recipe)&&!Array.isArray(plan.flowers))errors.push('缺少材料清单或旧版花材数据');
  if(!plan.dimensions||!['height','width','depth'].every(k=>finite(plan.dimensions[k])&&Number(plan.dimensions[k])>0&&Number(plan.dimensions[k])<=1000))errors.push('缺少有效的作品尺寸');
  if(!plan.vessel||!plan.mechanics||!Array.isArray(plan.mechanics.items))errors.push('缺少花器或固定与供水信息');
  if(plan.blueprint?.nodes!==undefined&&!Array.isArray(plan.blueprint.nodes))errors.push('施工点位必须是数组');
  if(plan.media!==undefined&&!Array.isArray(plan.media))errors.push('图片记录必须是数组');
  if(raw.plan&&raw.media&&raw.media.version!==1)errors.push('不支持的图片附件版本');
  if(plan.recipe!==undefined && !Array.isArray(plan.recipe))errors.push('Recipe 必须是数组');
  const recipeKeys=new Set();
  for(const r of Array.isArray(plan.recipe)?plan.recipe:[]){if(!r||typeof r!=='object'){errors.push('材料行不完整');continue;}if(!r.key||recipeKeys.has(r.key))errors.push('材料缺少 key 或 key 重复');recipeKeys.add(r.key);for(const k of ['quantity','owned','unit_price'])if(r[k]!==undefined&&r[k]!==null&&(!finite(r[k])||Number(r[k])<0))errors.push(`${r.name||r.key||'材料'} 的 ${k} 不是合法非负数值`);}
  ...
  const nodes=Array.isArray(plan.blueprint?.nodes)?plan.blueprint.nodes:[];const ids=new Set();for(const n of nodes){if(!n||typeof n!=='object'){errors.push('结构节点不完整');continue;}if(typeof n.id!=='string'||! /^[a-zA-Z0-9_-]{1,100}$/.test(n.id))errors.push('结构节点 ID 无效');else if(ids.has(n.id))errors.push(`重复 Stem ID：${n.id}`);else ids.add(n.id);for(const k of ['x','y','z'])if(!finite(n[k]))errors.push(`${n.id||'节点'} 的 ${k} 坐标非法`);if(n.material_id && !catalog.flowers.some(f=>f.id===n.material_id)&&!catalog.creative.some(c=>c.id===n.material_id))warnings.push(`${n.id} 使用未收录材料 ${n.material_id}`);}
