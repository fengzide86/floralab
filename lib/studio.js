...
function handoffObject(plan){return {schema:V.HANDOFF_SCHEMA,project:{id:plan.id,title:titleOf(plan),version:plan.handoff?.version||1},intent:{idea:plan.creativeBrief?.idea||'',mode:plan.mode,type:plan.type,occasion:plan.occasion,style:plan.style,palette:plan.palette,budget:plan.request?.budget||plan.cost?.budget||null,avoid:plan.request?.avoid||'',pet_context:plan.request?.petContext||''},assessment:plan.assessment,composition:plan.composition,compositionIntent:plan.compositionIntent||null,creativeBrief:plan.creativeBrief||null,exploration:plan.exploration,recipe:plan.recipe,dimensions:plan.dimensions,mechanics:plan.mechanics,blueprint:plan.blueprint,render:buildRenderSpec(plan),build:plan.build,resultFeedback:plan.resultFeedback,history:plan.history,plan};}
