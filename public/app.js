...
function renderSpecFor(p){return window.FloraLabRuntime?.Studio?.buildRenderSpec?.(p)||null;}
function renderRenderView(p,spec){
  const bp=p.blueprint;if(!bp||!bp.nodes?.length||!spec)return '<div class="empty-state">暂无可视结构</div>';
  const W=620,H=520,pad=52,d=bp.dimensions||p.dimensions||{width:60,height:56},sx=(W-pad*2)/(Number(d.width||60)||1),sy=(H-pad*2)/(Number(d.height||56)||1),scale=Math.min(sx,sy),cx=W/2,base=H-pad;
  const b=spec.structure.silhouette.bounds_cm,rx=cx+b.left*scale,ry=base-b.top*scale,rw=Math.max(24,(b.right-b.left)*scale),rh=Math.max(24,(b.top-b.bottom)*scale);
  const focus=new Set(spec.structure.focus_nodes.map(x=>x.id));
  const vessel=bp.vessel||{},vesselSvg=`<path d="M ${cx-50} ${base} L ${cx-38} ${base-Math.min(95,(vessel.height_cm||20)*scale)} Q ${cx} ${base-Math.min(108,(vessel.height_cm||20)*scale)-14} ${cx+38} ${base-Math.min(95,(vessel.height_cm||20)*scale)} L ${cx+50} ${base}" class="render-vessel"/>`;
  const anchor=(bp.mechanics?.anchors||[])[0]||{x:0,z:0},ax=cx+Number(anchor.x||0)*scale,ay=base-Number(anchor.z||0)*scale;
  const nodes=bp.nodes.map(n=>{const x=cx+Number(n.x||0)*scale,y=base-Number(n.z||0)*scale,f=focus.has(n.id),special=n.kind==='creative',r=special?8:f?7:4.8,fill=special?'#efe3d7':colorHex((n.color||'').includes('色')?n.color:`${n.color}色`,0);return `<g class="render-node ${f?'focus':''} ${special?'special':''}"><line x1="${ax}" y1="${ay}" x2="${x.toFixed(1)}" y2="${y.toFixed(1)}"/>${f?`<circle class="render-focus-halo" cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${r+7}"/>`:''}<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${r}" fill="${fill}"/><title>${esc(n.id)} · ${esc(n.name)} · ${esc(n.role||'')}</title></g>`;}).join('');
...
function renderHandoffTab(p){
  const r=renderSpecFor(p);if(!r)return '<section class="panel"><div class="empty-state">效果图交接暂不可用</div></section>';
