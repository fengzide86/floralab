(function () {
  ...
  function create({ state: S, esc, colorHex, viewNames: VIEW_NAMES }) {
    ...
    function renderBlueprint(
      ...
      const lines = nodes
        .map((n) => {
          ...
          return `<g class="bp-node ${selected ? 'selected' : ''} ${n.locked ? 'locked' : ''}" data-node="${esc(n.id)}" tabindex="0" aria-label="${esc(n.id+' '+n.name)}"><line x1="${ax}" y1="${ay}" x2="${x.toFixed(1)}" y2="${y.toFixed(1)}" stroke="${selected ? '#416b38' : '#9b948b'}" stroke-opacity="${n.kind === 'creative' ? 0.5 : 0.58}" stroke-width="${selected ? 1.8 : 0.8}"/><circle class="bp-hit" cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${interactive ? Math.max(40, r) : r}" fill="transparent"/><circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${r}" fill="${n.kind === 'creative' ? '#eee7df' : fill}" fill-opacity="${selected ? 0.96 : 0.76}" stroke="${selected ? '#416b38' : '#6f6861'}" stroke-width="${selected ? 1.6 : 0.65}"/><text class="bp-label" x="${(x + r + 3).toFixed(1)}" y="${(y + 3).toFixed(1)}" font-size="10.5" fill="${selected ? '#416b38' : '#6f6861'}">${n.id}</text></g>`;
        })
        .join('');
    ...
    function nodeInfo(n) {
      ...
      return `<span class="node-id">${esc(n.id)}</span><p class="node-description">${n.x<-5?'偏左':n.x>5?'偏右':'居中'} · ${n.y<-5?'靠后':n.y>5?'靠前':'前后居中'} · 高度约 ${n.z} cm</p><b>${esc(n.name)} · ${esc(n.color || n.role)}</b><dl><dt>长度</dt><dd>约 ${n.length_cm} cm</dd><dt>位置</dt><dd>x ${n.x} / y ${n.y} / z ${n.z}</dd><dt>朝向</dt><dd>水平 ${n.yaw_deg}° / 抬升 ${n.pitch_deg}°</dd><dt>层级</dt><dd>${esc(n.role)} · 第 ${n.stage} 阶段</dd></dl>`;
    }
