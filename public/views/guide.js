(function () {
  ...
  function open(topic = 'start') {
    ...
    dialog.innerHTML = `<header><div><span class="kicker">FloraLab · 使用说明</span><h2 id="guideTitle">从想法到成品，怎么走？</h2></div><button type="button" data-close-guide aria-label="关闭使用说明">关闭</button></header>
      ...
      <details ${topic==='start'?'open':''}><summary>1. 写想法，核对第一份方案</summary><p>点首页“开始一件作品”，写一句想法，例如“做一件白绿桌花，不要玫瑰”。已确定的颜色、材料和数量也可以补充。点“整理想法，查看方案”。</p><p>总览显示原文和识别结果。这是规则生成的起点，先核对材料、颜色、数量和形式；复杂气质与构图仍需要你判断。</p></details>
      ...
      <details><summary>6. 记录成品，保存与换设备</summary><p>在“成品与记录”添加实拍照片、耗时和体会，再保存成品记录。比较轮廓、颜色、层次和材料差异，不用虚构还原率。</p><p><strong>作品和图片保存在当前浏览器，没有账号云同步。</strong>等页面显示“已保存在本机”。换设备或清理浏览器之前，点“导出设计与图片”保存 .floralab 文件；另一台设备在首页导入，导入会另建作品。</p><p>误改时到“修改与恢复”预览旧快照再恢复；恢复前也会保留当前状态。</p></details>
      <details><summary>遇到点不动、拖不动怎么办？</summary><p>先确认在“摆放施工图”，再检查点位是否锁定。密集的花头优先用点位列表选择；拖动困难时直接用空间按钮。若已到尺寸边界，继续向外移不会生效。</p><p>若保存失败，先保留当前内容并导出。升级后旧页面不响应时，先复制尚未提交的文字，再联网刷新一次；不要为了刷新版本清除网站数据。</p></details>`;
