...
with sync_playwright() as p:
    ...
    # D03 Overview
    page.click('#generate');page.wait_for_selector('.project-title');page.wait_for_timeout(250)
...
report={'version':'1.2.0','checks':len(checks),'items':checks,'screenshots':27,'desktop':13,'mobile':14,'note':'Screenshots are primary page-by-page visual acceptance inputs. Contact sheet is intentionally not generated by this gate.'}
(QA/'ui-1.2.json').write_text(json.dumps(report,ensure_ascii=False,indent=2))
print(f"ui-1.2: {len(checks)} checks passed; 27 page screenshots written")
