...
with sync_playwright() as p:
    ...
    shot(page,'1.2-D20-new-bottom.png')
    ...
    shot(page,'1.2-D21-overview-bottom.png')
    ...
    shot(page,'1.2-D05-structure.png')
    ...
    shot(page,'1.2-D17-creative-space.png')
    ...
    shot(page,'1.2-D08-library-search.png')
    ...
    shot(page,'1.2-D18-core-reality.png')
    ...
    shot(page,'1.2-D11-reference-detail.png')
    ...
    shot(page,'1.2-D12-creative-detail.png')
    page.click('[data-library-back]');page.wait_for_selector('.library-shell')
    ...
    shot(page,'1.2-D19-creative-safety.png')
    ...
    ck(overflow_ok(mob),'M04 no overflow');shot(mob,'1.2-M04-recipe.png')
    # M05
    goto_tab(mob,'structure');mob.wait_for_selector('.blueprint-svg')
    ck(overflow_ok(mob),'M05 no overflow');shot(mob,'1.2-M05-structure.png')
    mob.locator('.node-info').scroll_into_view_if_needed();mob.wait_for_timeout(100)
    shot(mob,'1.2-M22-structure-inspector.png')
    # M06
    ...
    shot(mob,'1.2-M18-creative-space.png')
    ...
    shot(mob,'1.2-M09-library-search.png')
    # M10
    ...
    ck(overflow_ok(mob),'M14 no overflow');shot(mob,'1.2-M14-empty.png')
...
report={'version':'1.2.0','mode':'online' if LIVE_URL else 'build','base_url':LIVE_URL or 'in-memory-build','checks':len(checks),'items':checks,'screenshots':44,'desktop':21,'mobile':23,'note':'Screenshots are primary page-by-page visual acceptance inputs. Contact sheet is intentionally not generated by this gate.'}
(QA/'ui-1.2.json').write_text(json.dumps(report,ensure_ascii=False,indent=2))
print("ui-1.2 (%s): %d checks passed; 44 page screenshots written" % ("online" if LIVE_URL else "build",len(checks)))
