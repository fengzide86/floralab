...
report={'version':'1.3.0','mode':'online' if LIVE_URL else 'build','base_url':LIVE_URL or 'in-memory-build','checks':len(checks),'items':checks,'screenshots':47,'desktop':22,'mobile':25,'note':'Screenshots are primary page-by-page visual acceptance inputs. Contact sheet is intentionally not generated by this gate.'}
(QA/'ui-1.3.json').write_text(json.dumps(report,ensure_ascii=False,indent=2))
print("ui-1.3 (%s): %d checks passed; 47 page screenshots written" % ("online" if LIVE_URL else "build",len(checks)))
