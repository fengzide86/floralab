...
with sync_playwright() as p:
    ...
    shot(page,'1.3-D21-overview-bottom.png')
    ...
    shot(page,'1.3-D05-structure.png')
    ...
    shot(page,'1.3-D17-creative-space.png')
    ...
    shot(page,'1.3-D07-library-home.png')
    ...
    shot(page,'1.3-D08-library-search.png')
    ...
    shot(page,'1.3-D09-library-filter.png')
    ...
    shot(page,'1.3-D11-reference-detail.png')

    # D12 Creative detail
    page.click('[data-library-back]');page.wait_for_selector('.library-shell')
    page.click('[data-library-kind="creative"]');page.wait_for_timeout(120)
    page.click('[data-material-detail="creative:plush"]');page.wait_for_selector('.creative-detail')
    ck(page.locator('.creative-fix-list').count()==1,'D12 creative fixing guidance')
    ck(overflow_ok(page),'D12 no overflow')
    shot(page,'1.3-D12-creative-detail.png')
    page.click('[data-library-back]');page.wait_for_selector('.library-shell')
    page.click('[data-material-detail="creative:chocolate"]');page.wait_for_selector('.creative-detail')
    ...
    shot(page,'1.3-D19-creative-safety.png')
    ...
    mob.click('#navMore');mob.wait_for_timeout(80)
    ck(overflow_ok(mob),'M01 no overflow');shot(mob,'1.3-M01-home.png')
    # M02
    mob.click('#newDesign');mob.wait_for_selector('#generate')
    ck(px(mob,'.form-label span')>=14,'M02 form support readable');shot(mob,'1.3-M02-new.png')
    mob.locator('#generate').scroll_into_view_if_needed();mob.wait_for_timeout(100)
    shot(mob,'1.3-M20-new-bottom.png')
    # M03
    mob.click('#generate');mob.wait_for_selector('.project-title');mob.wait_for_timeout(200)
    ck(overflow_ok(mob),'M03 no overflow');shot(mob,'1.3-M03-overview.png')
    mob.locator('.next-link').scroll_into_view_if_needed();mob.wait_for_timeout(100)
    shot(mob,'1.3-M21-overview-bottom.png')
    # M04
    ...
    ck(overflow_ok(mob),'M04 no overflow');shot(mob,'1.3-M04-recipe.png')
    # M05
    goto_tab(mob,'structure');mob.wait_for_selector('.blueprint-svg')
    ck(overflow_ok(mob),'M05 no overflow');shot(mob,'1.3-M05-structure.png')
    mob.locator('.node-info').scroll_into_view_if_needed();mob.wait_for_timeout(100)
    shot(mob,'1.3-M22-structure-inspector.png')
    # M05R Render Handoff
    ...
    shot(mob,'1.3-M18-creative-space.png')
    ...
    ck(overflow_ok(mob),'M08 no overflow');shot(mob,'1.3-M08-library-home.png')
    # M09
    ...
    shot(mob,'1.3-M09-library-search.png')
    # M10
    ...
    shot(mob,'1.3-M10-filter.png')
    # M11
    mob.click('[data-material-detail="flower:hydrangea"]');mob.wait_for_selector('.material-detail-layout')
    ck(px(mob,'.material-detail-title h1')>=36,'M11 detail title')
    ck(overflow_ok(mob),'M11 no overflow');shot(mob,'1.3-M11-core-detail.png')
    # M12 safety
    ...
    shot(mob,'1.3-M12-safety-detail.png')
    # M13 creative
    mob.click('[data-library-back]');mob.wait_for_selector('.library-shell')
    ...
    mob.locator('.material-trust.safety').scroll_into_view_if_needed();mob.wait_for_timeout(100)
    shot(mob,'1.3-M13-creative-detail.png')
    ...
    ck(overflow_ok(mob),'M14 no overflow');shot(mob,'1.3-M14-empty.png')
...
report={'version':'1.3.1','mode':'online' if LIVE_URL else 'build','base_url':LIVE_URL or 'in-memory-build','checks':len(checks),'items':checks,'screenshots':48,'desktop':23,'mobile':25,'note':'Screenshots are primary page-by-page visual acceptance inputs. Contact sheet is intentionally not generated by this gate.'}
(QA/'ui-1.3.json').write_text(json.dumps(report,ensure_ascii=False,indent=2))
print("ui-1.3 (%s): %d checks passed; 48 page screenshots written" % ("online" if LIVE_URL else "build",len(checks)))
