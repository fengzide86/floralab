...
with sync_playwright() as p:
    ...
    shot(page,'1.4-D02-new.png')
    page.locator('#generate').scroll_into_view_if_needed();page.wait_for_timeout(100)
    shot(page,'1.4-D20-new-bottom.png')

    # D23-D25 Direction workspace (1.4)
    ...
    shot(page,'1.4-D23-direction.png')
    ...
    shot(page,'1.4-D24-variations.png')
    ...
    shot(page,'1.4-D25-branch-compare.png')
    ...
    shot(page,'1.4-D05-structure.png')
    ...
    shot(page,'1.4-D17-creative-space.png')
    ...
    shot(page,'1.4-D07-library-home.png')
    ...
    shot(page,'1.4-D08-library-search.png')
    ...
    shot(page,'1.4-D09-library-filter.png')
    ...
    shot(page,'1.4-D11-reference-detail.png')
    ...
    shot(page,'1.4-D12-creative-detail.png')
    page.click('[data-library-back]');page.wait_for_selector('.library-shell')
    ...
    shot(page,'1.4-D19-creative-safety.png')
    ...
    mob.click('#navMore');mob.wait_for_timeout(80)
    ck(overflow_ok(mob),'M01 no overflow');shot(mob,'1.4-M01-home.png')
    # M02
    mob.click('#newDesign');mob.wait_for_selector('#generate')
    ck(px(mob,'.form-label span')>=14,'M02 form support readable');shot(mob,'1.4-M02-new.png')
    mob.locator('#generate').scroll_into_view_if_needed();mob.wait_for_timeout(100)
    shot(mob,'1.4-M20-new-bottom.png')
    # M25-M27 Direction workspace (1.4)
    ...
    shot(mob,'1.4-M25-direction.png')
    ...
    shot(mob,'1.4-M26-variations.png')
    ...
    shot(mob,'1.4-M27-branch-compare.png')
    ...
    ck(overflow_ok(mob),'M03 no overflow');shot(mob,'1.4-M03-overview.png')
    mob.locator('.next-link').scroll_into_view_if_needed();mob.wait_for_timeout(100)
    shot(mob,'1.4-M21-overview-bottom.png')
    # M04
    ...
    ck(overflow_ok(mob),'M04 no overflow');shot(mob,'1.4-M04-recipe.png')
    # M05
    goto_tab(mob,'structure');mob.wait_for_selector('.blueprint-svg')
    ck(overflow_ok(mob),'M05 no overflow');shot(mob,'1.4-M05-structure.png')
    mob.locator('.node-info').scroll_into_view_if_needed();mob.wait_for_timeout(100)
    shot(mob,'1.4-M22-structure-inspector.png')
    # M05R Render Handoff
    ...
    shot(mob,'1.4-M18-creative-space.png')
    ...
    ck(overflow_ok(mob),'M08 no overflow');shot(mob,'1.4-M08-library-home.png')
    # M09
    ...
    shot(mob,'1.4-M09-library-search.png')
    # M10
    ...
    ck(overflow_ok(mob),'M11 no overflow');shot(mob,'1.4-M11-core-detail.png')
    # M12 safety
    ...
    shot(mob,'1.4-M12-safety-detail.png')
    # M13 creative
    mob.click('[data-library-back]');mob.wait_for_selector('.library-shell')
    ...
    mob.locator('.material-trust.safety').scroll_into_view_if_needed();mob.wait_for_timeout(100)
    shot(mob,'1.4-M13-creative-detail.png')
    ...
    ck(overflow_ok(mob),'M14 no overflow');shot(mob,'1.4-M14-empty.png')
...
report={'version':'1.4.0','mode':'online' if LIVE_URL else 'build','base_url':LIVE_URL or 'in-memory-build','checks':len(checks),'items':checks,'screenshots':54,'desktop':26,'mobile':28,'note':'Screenshots are primary page-by-page visual acceptance inputs. Contact sheet is intentionally not generated by this gate.'}
(QA/'ui-1.4.json').write_text(json.dumps(report,ensure_ascii=False,indent=2))
print("ui-1.4 (%s): %d checks passed; 54 page screenshots written" % ("online" if LIVE_URL else "build",len(checks)))
